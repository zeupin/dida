...
abstract class Routing
{
     ...
     * @return void
     */
    abstract public function setDefaultPathInfo();
     ...
     * 匹配路由.
     *
     * 如果匹配成功,$this->matchError=['code'=>0, 'msg'=>''],然后将匹配到的数据存入$this->matchInfo.
     * 如果匹配失败,$this->matchError=['code'=>错误码, 'msg'=>'错误原因'].
     *
    ...
    public function start()
    {
        ...
        // 如果match()失败
        if ($this->match() === false) {
            throw new Exception('Routing match() fail.', Routing::MATCH_EXCEPTION);
        }

        // 如果check()失败
        if ($this->check() === false) {
            throw new Exception('Routing match() fail.', Routing::CHECK_EXCEPTION);
        }

        // 如果execute()失败
        if ($this->execute() === false && $this->executeError['code'] !== 0) {
            throw new Exception('Routing match() fail.', Routing::EXECUTE_EXCEPTION);
        }
    ...
    public function check()
    {
        ...
        if (!$callback) {
            return [
                'code' => Routing::ERROR_CALLBACK_INVALID,
        ...
        if (!is_array($callback)) {
            return [
                'code' => Routing::ERROR_CALLBACK_INVALID_TYPE,
        ...
        if (!class_exists($controller)) {
            return [
                'code' => Routing::ERROR_CALLBACK_CONTROLLER_INVALID,
        ...
        if (!method_exists($controller, $action)) {
            if (method_exists($controller, '__call')) {
                ...
                return [
                    'code' => Routing::ERROR_CALLBACK_ACTION_INVALID,
